        ...
        <>
            <div className={`box-border relative mx-2 my-4 p-2 max-w-sm rounded overflow-hidden shadow-md  ${props.mode === 'dark' ? 'shadow-amber-500 bg-[#041C32]' : 'shadow-gray'} pb-6`}>
                <div className="px-2 md:px-6 text-sm md:text-base py-4">
                    ...
                    <p className={`rounded mt-6 font-semibold ${props.mode === 'dark' ? 'bg-green-200  text-black' : 'text-gray-700 bg-green-200 '}`}><small className="text-xs">{note.tag}</small></p>
                    ...
                    <div className='absolute bottom-0 right-0 p-2'>
                        ...
                        {/* <span className=' cursor-pointer p-1 hover:bg-slate-200' onClick={() => { deleteNote(note._id); props.showAlert("Deleted successfully!", "bg-green-200", "text-green-600", "Success") }}><Delete fontSize="small" color="error" /></span> */}
                    </div>
                </div>
            </div>
            <div className={confirmDelete ?`bg-gray-500/90 flex justify-center items-center overflow-x-hidden overflow-y-auto fixed inset-0 z-30 outline-none focus:outline-none` : 'hidden'}>
                <div className={`${props.mode==='dark'? 'bg-[#322F3D] text-white' : 'bg-white text-gray-700'} p-10 rounded-md text-center`}>
                    ...
                    <button className="bg-red-500 hover:bg-red-700 px-4 py-2 mr-2 rounded-md text-sm w-20 text-white font-semibold"  onClick={handleConfirmDelete}>Cancel</button>
                    <button className="bg-indigo-500 hover:bg-indigo-700 px-4 py-2 ml-2 rounded-md text-sm w-20 text-white font-semibold disabled:opacity-60 disabled:cursor-not-allowed"  onClick={() => { deleteNote(note._id); props.showAlert("Deleted successfully!", "bg-green-200", "text-green-600", "Success") }}>Confirm</button>
                </div>
            </div>
